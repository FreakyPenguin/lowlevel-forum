		...
		'columns' => array(
			...
			'ip' => array(
				...
				'data' => array(
					'function' => create_function('$rowData', '
						global $scripturl;
						$url = strtr($scripturl, array(\'%\' => \'%%\')) . \'?action=trackip;searchip=\' . $rowData[\'member_ip2\'];
						return sprintfspamer($rowData, $url, \'member_ip2\', 1);
					'),
				),
				'sort' => array(
					'default' => 'INET_ATON(member_ip2)',
					'reverse' => 'INET_ATON(member_ip2) DESC',
				),
			),
			'last_active' => array(
				'header' => array(
...
function MembersAwaitingActivation()
{
	...
	$context['columns'] = array(
		...
		'member_ip2' => array('label' => $txt['admin_browse_ip']),
			...
			'hostname' => array(
				...
				'data' => array(
					'function' => create_function('$rowData', '
						...
						return host_from_ip($rowData[\'member_ip2\']);
					'),
					'class' => 'smalltext',
				),
			),
