...
function loadcheckedMembers_1($users)
{
	...
	$row = array();
	$resource = db_query("
		SELECT ID_MEMBER AS id_member, memberName AS member_name, emailAddress AS email_address, member_ip2 AS member_ip, is_spammer
...
function loadcheckedMembers_2($users)
{
	...
	$resource = $smcFunc['db_query']('', '
		SELECT id_member, member_name, email_address, member_ip2 AS member_ip, is_spammer
			FROM {db_prefix}members
