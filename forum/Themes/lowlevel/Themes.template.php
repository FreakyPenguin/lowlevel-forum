...
function template_list_themes()
{
	...
	foreach ($context['themes'] as $theme)
	{
		echo '
			...
			if ($theme['id'] != 1)
				echo '
					<span class="floatright"><a href="', $scripturl, '?action=admin;area=theme;sa=remove;th=', $theme['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['theme_remove_confirm'], '\');"><img src="', $settings['images_url'], '/icons/delete_theme.png" alt="', $txt['theme_remove'], '" title="', $txt['theme_remove'], '" /></a></span>';
...
function template_pick()
{
	...
	$count_themes = count($context['available_themes']);
	$i = 0;
	...
	foreach ($context['available_themes'] as $theme)
	{
		...
		echo '
				...
				<span class="botslice ', $count_themes == $i ? ' last_theme' : '', '"><span></span></span>
			</div>';
