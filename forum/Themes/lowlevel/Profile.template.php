...
<div id="profileview" class="flow_auto">
	...
	{
		echo '
					<dt class="warning_level">', $txt['profile_warning_level'], ': </dt>
					<dd>
	...
	{
		if (!empty($context['member']['ip2']))
		echo '
					<dt>', $txt['ip'], ': </dt>
					<dd><a href="', $scripturl, '?action=profile;area=tracking;sa=ip;searchip=', $context['member']['ip2'], ';u=', $context['member']['id'], '">', $context['member']['ip2'], '</a></dd>';

		if (empty($modSettings['disableHostnameLookup']) && !empty($context['member']['ip2']))
			echo '
					<dt>', $txt['hostname'], ': </dt>
...
function template_showPosts()
{
	...
	echo '
		...
		<div class="', (!isset($context['attachments']) ? 'pagesection' : 'pagesection_top'), '">
			<span>', $txt['pages'], ': ', $context['page_index'], '</span>
		</div>';

	// Button shortcuts
	$quote_button = create_buttons('quote.png', 'reply_quote', 'quote', 'align="middle"');
	$reply_button = create_buttons('reply.png', 'reply', 'reply', 'align="middle"');
	$remove_button = create_buttons('delete.png', 'remove_message', 'remove', 'align="middle"');
	$notify_button = create_buttons('notify.png', 'notify_replies', 'notify', 'align="middle"');
	...
	if (!isset($context['attachments']))
	{
		...
		<div class="topic">
			...
			if ($post['can_reply'])
				echo '
						<li class="reply_button"><a href="', $scripturl, '?action=post;topic=', $post['topic'], '.', $post['start'], '">', $reply_button, '</a></li>';


			// If they *can* quote?
			if ($post['can_quote'])
				echo '
						<li class="quote_button"><a href="', $scripturl . '?action=post;topic=', $post['topic'], '.', $post['start'], ';quote=', $post['id'], '">', $quote_button, '</a></li>';

			// Can we request notification of topics?
			if ($post['can_mark_notify'])
				echo '
						<li class="notify_button"><a href="', $scripturl, '?action=notify;topic=', $post['topic'], '.', $post['start'], '">', $notify_button, '</a></li>';

			// How about... even... remove it entirely?!
			if ($post['can_delete'])
				echo '
						<li class="remove_button"><a href="', $scripturl, '?action=deletemsg;msg=', $post['id'], ';topic=', $post['topic'], ';profile;u=', $context['member']['id'], ';start=', $context['start'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');">', $remove_button, '</a></li>';
	...
	foreach ($context['buddies'] as $buddy)
	{
		echo '
			<tr class="', $alternate ? 'windowbg' : 'windowbg2', '">
				...
				<td align="center"><a href="', $scripturl, '?action=profile;area=lists;sa=buddies;u=', $context['id_member'], ';remove=', $buddy['id'], ';', $context['session_var'], '=', $context['session_id'], '"><img src="', $settings['images_url'], '/icons/profile_delete.png" alt="', $txt['buddy_remove'], '" title="', $txt['buddy_remove'], '" /></a></td>
			</tr>';
...
function template_editIgnoreList()
{
	...
	echo '
		<div class="title_bar">
			<h3 class="titlebg">
				<span class="ie6_header floatleft"><img src="', $settings['images_url'], '/icons/profile_ignore_sm.png" alt="" class="icon" />', $txt['editIgnoreList'], '</span>
			</h3>
		</div>
		<table border="0" width="100%" cellspacing="1" cellpadding="4" class="table_grid" align="center">
			<tr class="catbg">
	...
	foreach ($context['ignore_list'] as $member)
	{
		echo '
			<tr class="', $alternate ? 'windowbg' : 'windowbg2', '">
				...
				<td align="center"><a href="', $scripturl, '?action=profile;u=', $context['id_member'], ';area=lists;sa=ignore;remove=', $member['id'], ';', $context['session_var'], '=', $context['session_id'], '"><img src="', $settings['images_url'], '/icons/profile_delete.png" alt="', $txt['ignore_remove'], '" title="', $txt['ignore_remove'], '" /></a></td>
			</tr>';
	...
	echo '
		...
		<div class="windowbg2">
			...
			<form action="', $context['base_url'], '" method="post" accept-charset="', $context['character_set'], '">
				<div class="padding content">', $txt['enter_ip'], ':&nbsp;&nbsp;<input type="text" name="searchip" value="', $context['ip'], '" class="input_text" />&nbsp;&nbsp;<input type="submit" value="', $txt['trackIP'], '" class="button_submit" /></div>
			</form>
			<span class="botslice"><span></span></span>
		</div>
		<br />';
...
function template_statPanel()
{
	...
	echo '
		<div class="flow_hidden">
			<div id="popularposts">
				<div class="cat_bar">
					<h3 class="catbg">
						<span class="ie6_header floatleft"><img src="', $settings['images_url'], '/stats_pop_boards.gif" alt="" class="icon" />', $txt['statPanel_topBoards'], '</span>
					</h3>
				</div>
				<div class="windowbg2">
					<span class="topslice"><span></span></span>
	...
	else
	{
		...
		foreach ($context['popular_boards'] as $board)
		{
			echo '
							<dt>', $board['link'], '</dt>
							<dd>
								<div class="profile_pie" style="width: ', ((int) $board['posts_percent']), '%;" title="', sprintf($txt['statPanel_topBoards_memberposts'], $board['posts'], $board['total_posts_member'], $board['posts_percent']), '">
	...
	echo '
			<div id="popularactivity">
				<div class="cat_bar">
					<h3 class="catbg">
					<span class="ie6_header floatleft"><img src="', $settings['images_url'], '/stats_pop_boards.gif" alt="" class="icon" />', $txt['statPanel_topBoardsActivity'], '</span>
					</h3>
				</div>
				<div class="windowbg2">
					<span class="topslice"><span></span></span>
	...
	else
	{
		...
		foreach ($context['board_activity'] as $activity)
		{
			echo '
							<dt>', $activity['link'], '</dt>
							<dd>
								<div class="profile_pie" style="width: ', ((int) ($activity['percent']) / 10 * 5), '%;" title="', sprintf($txt['statPanel_topBoards_posts'], $activity['posts'], $activity['total_posts'], $activity['posts_percent']), '">
...
function template_issueWarning()
{
	...
	echo '
		...
		<div class="pagesection_bottom">', $txt['pages'], ': ', $context['page_index'], '</div>';
...
function template_deleteAccount()
{
	...
	echo '
		<form action="', $scripturl, '?action=profile;area=deleteaccount;save" method="post" accept-charset="', $context['character_set'], '" name="creator" id="creator">
			<div class="title_bar">
				<h3 class="titlebg">
					<span class="ie6_header floatleft"><img src="', $settings['images_url'], '/icons/profile_delete.png" alt="" class="icon" />', $txt['deleteAccount'], '</span>
				</h3>
			</div>';
